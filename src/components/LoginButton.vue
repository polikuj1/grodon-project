      ...
      <div class="error-content">
        ...
        <div v-if="showCorsHelp" class="error-help">
          ...
          <div class="fix-section">
            ...
            <ol>
              <li>打開 <a href="https://console.cloud.google.com/" target="_blank">Google Cloud Console</a></li>
              <li>APIs & Services → Credentials</li>
              <li>編輯 OAuth 2.0 客戶端 ID</li>
              <li>在「授權的 JavaScript 來源」只添加：
...
http://127.0.0.1:5173
        ...
        <div class="error-actions">
          ...
          <button @click="checkConsole" class="console-button">查看 Console 詳細診斷</button>
        </div>
      </div>
    </div>
...
const showCorsHelp = computed(() => {
  return errorMessage.value.includes('CORS') || 
...
const redirectToLocalhost = () => {
  ...
  window.location.href = `http://localhost:${currentUrl.port || '5173'}${currentUrl.pathname}${currentUrl.search}`
}

// 重定向到 127.0.0.1
const redirectTo127 = () => {
  const currentUrl = new URL(window.location.href)
  window.location.href = `http://127.0.0.1:${currentUrl.port || '5173'}${currentUrl.pathname}${currentUrl.search}`
}

// 提示用戶查看 Console
const checkConsole = () => {
  alert('請打開瀏覽器開發者工具 (F12) → Console 標籤，查看詳細的診斷資訊和修復指引。')
}
